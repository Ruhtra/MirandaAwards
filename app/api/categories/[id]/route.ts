...
export async function GET(request: Request, { params }: { params: { id: string } }): Promise<NextResponse<CategoryDTO | { error: string }>> {
    try {
    ...
    try {
        ...
        if (!id) {
            return NextResponse.json(
                { error: "ID da category é obrigatório" },
                { status: 400 }
            )
        }
        ...
        if (!name) {
            return NextResponse.json(
                { error: "Nome da category é obrigatório" },
                { status: 400 }
            )
        }
        ...
        return NextResponse.json(
            { error: "Erro ao atualizar category" },
            { status: 500 }
        )
    }
}
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
    try {
        ...
        if (!categoryWithGames) return NextResponse.json(
            { error: "Category não encontrada" },
            { status: 404 }
        )


        if (categoryWithGames._count.games > 0) return NextResponse.json(
